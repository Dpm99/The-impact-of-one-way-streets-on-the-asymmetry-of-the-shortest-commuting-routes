...
result=[2 if i in [14, 22, 28] else 1 for i in range(30)]
cities = ox.geocode_to_gdf(['Berlin', 'Potsdam', 'Falkensee', 'Hennigsdorf', 'Hohen Neuendorf',
                           'Ahrensfelde', 'Neuenhagen bei Berlin', 'Schöneiche bei Berlin', 'Erkner',
                           'Schönefeld', 'Stahnsdorf', 'Kleinmachnow', 'Teltow', 'Dahlwitz-Hoppegarten',
                           'Woltersdorf', 'Gosen-Neu Zittau', 'Königs Wusterhausen',
                           'Zeuthen', 'Eichwalde', 'Schulzendorf', 'Großbeeren',
                           'Mahlow', 'Schönwalde-Siedlung', 'Dallgow-Döberitz',
                           'Hönow', 'Panketal', 'Mühlenbecker Land', 'Schönerlinde',
                           'Schönwalde', 'Glienicke/Nordbahn'], which_result=result)
...
#    fig.savefig('/Shuffle_maps/Full_'+city_name+'_shuffled_new_algorithm_'+str(number)+'_teste.png')
...
#    fig.set_frameon(True)
#    fig.tight_layout()
##    fig.savefig('/Shuffle_maps/Paris_shuffled_new_algorithm_'+str(number)+'.png')
#    fig.savefig('/Shuffle_maps/'+city_name+'_shuffled_new_algorithm_'+str(number)+'_teste.png')
    ...
    df_paths.to_csv('/Shuffle_data/'+city_name+'_teste/'+city_name+'_100k_paths_shuffle_'+str(number)+'.dat')
