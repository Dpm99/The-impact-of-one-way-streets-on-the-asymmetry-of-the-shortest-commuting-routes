...
ox.config(log_console=True, use_cache=True, cache_folder = '/home/hygor/Dropbox/Pesquisa/Projetos/Diogo_Rede_simetria/Beta_Experiment/Network_Data') ## Path da pasta onde estao os jsons
...
## Load Data
city_name = 'Paris'
result = [1 if i!=29 else 5 for i in range(30)]
cities = ox.geocode_to_gdf(['France, Paris', 'Montrouge', 'Malakoff', 'Vanves', 'Issy-les-Moulineaux', 
                           'Boulogne-Billancourt', 'Neuilly-sur-Seine', 'Levallois-Perret', 'Suresnes',
                           'Puteaux', 'Saint-Cloud', 'Clichy', 'Saint-Ouen-sur-Seine', 'Saint-Denis', 'Aubervilliers',
                           'Pantin', 'Le Pré-Saint-Gervais', 'Les Lilas', 'Bagnolet', 'Montreuil-sous-Bois', 'Vincennes',
                           'Fontenay-sous-Bois', 'Saint-Mandé', 'Nogent-sur-Marne', 'Joinville-le-Pont', 
                           'Charenton-le-Pont', 'Ivry-sur-Seine', 'Le Kremlin-Bicêtre', 'Gentilly', 'Saint Maurice'], which_result=result)
...
#    fig.savefig('/Shuffle_maps/Full_Paris_shuffled_new_algorithm_'+str(number)+'_teste.png')
...
#    fig.savefig('/Shuffle_maps/Paris_shuffled_new_algorithm_'+str(number)+'_teste.png')
    ...
    df_paths.to_csv('/Shuffle_data/'+city_name+'_teste2/'+city_name+'_100k_paths_shuffle_'+str(number)+'.dat') ## onde guardar o dataframe
    ...
    df_shuffle_network.to_csv('/Shuffle_data/'+city_name+'_teste2/'+city_name+'_shuffle_network_'+str(number)+'.dat')
    ...
    save.to_csv('/Shuffle_data/'+city_name+'_teste2/'+city_name+'_fractions_shuffle_'+str(number)+'.dat')
